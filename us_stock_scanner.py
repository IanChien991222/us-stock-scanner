...
def save_overview_card(results, trump):
    ...
    draw_text(draw, (70, 140), datetime.now(ZoneInfo("Asia/Taipei")).strftime("%Y/%m/%d %H:%M"), FONT_SM, MUTED)
    ...
    for i, r in enumerate(results[:5], 1):
        ...
        action_color = GREEN if "布局" in action else YELLOW
        ...
        draw_text(draw, (900, y + 18), action, FONT_MD, action_color)
        y += 84
...
def save_stock_card(rank, result):
    ...
    for sig in result["signals"][:3]:
        for line in textwrap.wrap(f"• {sig}", width=34):
            draw_text(draw, (595, y), line, FONT_SM)
            y += 34
        y += 8

    rounded_box(draw, (70, 540, 1210, 650), "#10233D", radius=22, outline=ACCENT, width=2)
...
def run_scan():
    print(f"[{datetime.now(ZoneInfo('Asia/Taipei')).strftime('%H:%M')}] Trump signal...")
