...
BREAKOUT_WATCHLIST = [
    "POET","SOUN","BBAI","RXRX","ACHR","JOBY","RKLB",
    "LUNR","ASTS","IONQ","RGTI","QUBT","HIMS","CELH",
    "AFRM","HOOD","SOFI","UPST","DKNG","SMCI","APLD",
    "BHVN","PRAX","KROS","TMDX","RELY","SKYH","OPEN",
    "CLOV","APP","RDDT","CART","KVYO","ARM","MSTR"
]
...
def send_telegram_photos(image_paths, caption=""):
    ...
    for i, path in enumerate(image_paths):
        ...
        if i == 0 and caption: data["caption"] = caption
...
def classify_action(result):
    ...
    if score >= 60 and rsi < 72: return "👀 考慮買入"
    if rsi >= 70:                 return "⚠️ 過熱觀望"
    if pct < -18:                 return "🔻 趨勢偏弱"
    return "➡️ 持續觀察"

def analyze_stock(ticker, mode="normal"):
    try:
        ...
        if 45 < rsi < 70:   tech_score += 15; signals.append(f"RSI {rsi:.0f} 健康區間")
        elif rsi >= 70:      tech_score += 5;  signals.append(f"RSI {rsi:.0f} 偏熱注意")
        elif rsi < 35:       tech_score += 10; signals.append(f"RSI {rsi:.0f} 超賣反彈")
        ...
        if vol_spike >= 2.0:
            tech_score = min(tech_score+8, 50); signals.append(f"爆量 {vol_spike:.1f}x ⚡")
        elif vol_spike >= 1.5:
            tech_score = min(tech_score+4, 50); signals.append(f"量增 {vol_spike:.1f}x")
        ...
        if mode == "breakout":
            ...
            if drawdown <= -30:
                ...
                if recovery >= 10 and rsi > 40:
                    tech_score += 20; pullback_signal = True
                    signals.append(f"⚡ 高點回落{drawdown:.0f}% 反彈+{recovery:.0f}%")
                    signals.append("POET型態：回落築底再入場")
                elif recovery >= 5:
                    tech_score += 10
                    signals.append(f"回落{drawdown:.0f}% 初期反彈+{recovery:.0f}%（待確認）")
            if vol_spike >= 3.0:
                tech_score = min(tech_score+10, 55)
                signals.append(f"爆量突破訊號 {vol_spike:.1f}x")

        if eps_g and eps_g > 0.15: fund_score += 15; signals.append(f"EPS年增+{eps_g*100:.0f}%")
        elif eps_g and eps_g > 0:  fund_score += 8;  signals.append(f"EPS年增+{eps_g*100:.0f}%")
        ...
        return {
            "ticker": ticker, "mode": mode, "close": close, "rsi": rsi, "atr": atr,
            "tech_score": tech_score, "fund_score": fund_score,
            "total_score": tech_score + fund_score,
            "signals": signals[:5], "stop_loss": stop_loss, "stop_loss_pct": stop_loss_pct,
            "sector": sector, "mktcap": mktcap, "pct_5d": pct_5d,
            "pct_from_52w_high": pct_from_52w_high, "vol_spike": vol_spike,
